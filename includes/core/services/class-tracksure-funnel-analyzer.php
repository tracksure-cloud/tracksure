...
class TrackSure_Funnel_Analyzer
{
	...
	public function get_conversion_paths($start_date, $end_date, $limit = 20)
	{
		...
		// Two-phase query:
		// 1) Inner: build per-conversion path strings by joining touchpoints → conversions on visitor_id.
		// 2) Outer: group identical paths and count how many conversions share each path.
		$sql = "
		SELECT
			conversion_path,
			...
			AVG(avg_sessions_to_convert) as avg_sessions_to_convert
		FROM (
			SELECT
				GROUP_CONCAT(
					CONCAT(
						COALESCE(NULLIF(t.utm_source, ''), '(direct)'),
						'/',
						COALESCE(NULLIF(t.utm_medium, ''), '(none)'),
						'/',
						COALESCE(NULLIF(t.channel, ''), 'unknown')
					)
					ORDER BY t.touchpoint_seq ASC
					SEPARATOR ' → '
				) as conversion_path,
				c.conversion_value as total_value,
				c.time_to_convert as avg_time_to_convert,
				c.sessions_to_convert as avg_sessions_to_convert
			FROM {$wpdb->prefix}tracksure_touchpoints t
			INNER JOIN {$wpdb->prefix}tracksure_conversions c
